...
{
    if (this->loop_cycle > 0)
        throw GameAlreadyStartedException("Can't set map after the game has started.");
    this->map = map;
}
...
void Game::putHero(Hero &hero, int x, int y)
{
    if (map.get(x, y) == Map::Wall)
        throw OccupiedException("Can't place entity on wall.");
    if (!isMapSet())
        throw WrongIndexException("Map is not set.");
    if (isHeroSet())
        throw AlreadyHasUnitsException("The hero is already set.");
    if (this->loop_cycle > 0)
        throw GameAlreadyStartedException("Can't set hero after the game has started.");
...
{
    if (!isMapSet())
        throw NotInitializedException("Map is not set.");
    if (!isHeroSet())
...
bool Game::areHeroesAlive()
{
    ...
    {
        if (std::get<Hero>(a.getEntity()).isAlive())
            return true;
    }
    return false;
}
